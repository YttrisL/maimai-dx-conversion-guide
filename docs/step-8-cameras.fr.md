...
* **Branchement :** Sur l'ordinateur *ALLS* posé à plat, cette webcam doit se brancher sur le port USB situé **en bas à droite** *([Manuel](pdfs/maimai-dx-manual-full.pdf), page 128)*.
  Deux LEDs dédiées sont également à câbler sur l'IO4. Sur le connecteur CN9, en position 7 "CAMERA LED WARM" et en position 8 "CAMERA LED RED". *(Voir [manuel](pdfs/maimai-dx-manual-full.pdf), page 194)*.
...
* **L'éclairage :** Le jeu exige d'éclairer ces cartes avec des LEDs blanches (alimentées en 12V). Ces LEDs sont contrôlées par l'IO4, via les broches 55 et 56. *(Voir [manuel](pdfs/maimai-dx-manual-full.pdf), page 194)*.

...
Dans ce guide, RS-232 est le protocole que le jeu utilise pour piloter les [contrôleurs de LEDs](step-7-lighting.md) et pour dialoguer avec les dalles tactiles *d'origine*, ainsi que pour récupérer l'information du lecteur Aime et dire quoi afficher au VFD. C'est un standard très utilisé dans le milieu de l'arcade. (Les dalles ADX de ce guide, elles, ne parlent pas RS-232 nativement — voir le point 5 ci-dessous.)

**Le connecteur DB9 :** c'est le petit connecteur trapézoïdal à 9 broches sur deux rangées que RS-232 a rendu quasi universel. On le confond souvent à tord avec un port VGA "inversé". C'est celui que l'on retrouve sur le Ringedge2 et sur le ALLS. Sur les 9 broches, seules trois nous intéressent vraiment ici : **TX** (émission), **RX** (réception) et **GND** (la masse, la référence commune de tension) — comme sur le schéma point à point vu plus haut. Les autres broches (DTR, DSR, RTS, CTS, DCD, RI) servent à des signaux de contrôle optionnels (négociation de flux, détection de porteuse...), rarement utilisés dans le domaine de l'arcade.
